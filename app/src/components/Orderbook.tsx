    ...
    return (
        <div className='card' style={{ flex: '1', minHeight: '465px', display: 'flex', justifyContent: 'start' }}>
            ...
            {orders.map((order: OrderRow, index: number) => (
                <div key={index} className='text' style={{margin: '0px'}}>
                    <div style={styleFlex}>{order.bidSize}</div>
                    <div className='clr-green' style={{ ...styleFlex, position: 'relative', ...styleTextEnd }}>
                        <span style={{ paddingRight: '5px' }}>{order.bidPrice}</span>
                        <div style={{ backgroundColor: '#00FF0040', height: '100%', width: order.bidPercent + '%', position: 'absolute', top: '0px' }}></div>
                    </div>
                    {/* <div className='clr-grey-dark'>|</div> */}
                    <div className='clr-red' style={{ ...styleFlex, position: 'relative' }}>
                        <span style={{ paddingLeft: '5px' }}>{order.askPrice}</span>
                        <div style={{ backgroundColor: '#FF000040', height: '100%', width: order.askPercent + '%', position: 'absolute', top: '0px' }}></div>
                    </div>
